...
def insert_bn(names: list):
    ...
    for name in names:
        ...
        if 'conv' in name:
            pos = name.replace('conv', '') # 替换conv5_2中的conv为空字符，取出数字即位置   # str.replace('a','b')替换str中的a为b，生成并返回一个新的字符串，源字符串是不可改变的
...
class VGGFeatureExtractor(nn.Module):
    ...
    def __init__(self,
        ...
        """
        torchvision 官方的 VGG 实现分两种版本：
        普通版（如vgg19）：features层顺序是 Conv → ReLU → Conv → ReLU → Pool...（无 BN）；
        BN 版（如vgg19_bn）：features层顺序是 Conv → BatchNorm → ReLU → Conv → BatchNorm → ReLU → Pool...（包含 BN 层）。
        所以我预先定义的names需要根据传入的vgg_type修改，但是features直接根据vgg_type选择即可
        """
    ...
    def forward(self, x):
        ...
        for key, layer in self.vgg_net._modules.items():
            x = layer(x)
            if key in self.layer_name_list: # 只提取指定层(defined in layer_name_list)的特征
                output[key] = x.clone()
